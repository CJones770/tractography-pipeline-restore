...
./disk_Check.sh $1 $3 . $4 #check if there is sufficient disk space to run the program, terminate if not
...
./min_eddy8.sh $1 #Correct for Eddy Currents and subject motion
...
./M_Tmp3_2_Out.sh $1 $2 #Move Temp outputs of interest to specified output directory #3 [Stg3Out]
#Stage 4
...
./M_Tmp4_2_Out.sh $1 $2 #Move Temp outputs of interest to specified output directory #4 [Stg4Out]
./M_xtract_gpu.sh $1 $2 #Perform tractography algorithm and store in specified output directory #5 [XtractOutDir]
